...
\section{Các kết quả thí nghiệm}
    ...
    Trong phần này, mục tiêu của  chúng tôi là kiểm chứng về sự phù hợp của Variational auto-encoder so với Auto-encoders, cụ thể là so sánh Mul-VAE và Mul-DAE.
    ...
    Còn ở thí nghiệm này, cụ thể thì trong tập kiểm tra chúng tôi sẽ lần lượt thực hiện giảm số lượng tương tác người dùng trong tập fold-in.
    Tập fold-in là tập dữ liệu đầu vào của mô hình, thể hiện các tương tác của người dùng trong quá khứ.
    Chúng tôi lần lượt đánh giá ở các mức độ tương tác khác nhau của người dùng.
    Mức độ tương tác đơn giản là số lượng tương tác trong quá khứ so với toàn bộ tương tác của người dùng được ghi nhận.       
    ...
    \begin{figure}
        ...
        \caption{So sánh Mul-VAE và Mul-DAE ở các mức độ tương tác khác nhau trên tập kiểm tra trên tập Million Song Datasets}
        \label{fig_vae_dae_msd}
    \end{figure}

    Hình \ref{fig_vae_dae_ml} thể hiện giá trị của độ đo NDCG@100 trên tập Movilen. 
    Với tập dữ liệu này, ta dễ dàng nhận thấy Mul-VAE vượt trội hơn so với Mul-DAE, đặc biệt là khi dữ liệu tương tác người dùng thấp.
    Cụ thể trong khoảng mức độ tương tác của người dùng từ 20\% đến 40\% thì Mul-DAE hoàn toàn bị bỏ xa bởi Mul-VAE.
    Còn ở hình \ref{fig_vae_dae_msd} là kết quả thực hiện thí nghiệm trên tập MSD.
    Tập dữ liệu này cũng có xu hướng tương tự như trên tập Movielen, tuy nhiên sự cách biệt giữa Mul-DAE và Mul-VAE không quá rõ ràng.
    Song ta vẫn dễ dàng nhận thấy được rằng với mức độ tương tác thấp Mul-VAE vẫn cho kết quả nhỉnh hơn so với Mul-DAE.

    Kết quả thí nghiệm giúp ta đã giúp khẳng định lại được độ hiệu quả của mô hình Mul-VAE được đề xuất trong bài báo \cite{mvae}. 
    Tuy trên tổng quan thì Mul-VAE không quá vượt trội so với Mul-DAE nhưng ở trường hợp dữ liệu tương tác người dùng ở mức độ thấp thì Mul-VAE mang lại kết quả cực kỳ ấn tượng.
